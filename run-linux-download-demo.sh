...
# and run the browser-initiated file downloads demo (WebViewDownloadDemo).
#
# Exercises `<a download>`, a `Content-Disposition: attachment` response,
# a body the engine will not render inline, and known- vs unknown-size
# progress reporting, across three handler modes (stock save dialog,
# silent programmatic handler, null = refuse every download).  See
# demos/WebViewDownloadDemo/README.md for the AC-mapped manual test
# checklist.
#
# Usage:    ./run-linux-download-demo.sh             # lightweight (default)
#           ./run-linux-download-demo.sh heavyweight # force heavyweight mode
#           ./run-linux-download-demo.sh lightweight # force lightweight mode
#           WEBVIEW_MODE=heavyweight ./run-linux-download-demo.sh
#
# STORY-006-002 requires AC verification in BOTH modes; run the script
# twice (once per mode) to cover the full acceptance matrix.
#
# Override JDK:  JAVA_HOME=/path/to/jdk ./run-linux-download-demo.sh
#
# Required packages:
#   - g++, pkg-config
#   - libgtk-3-dev
...
# ---------------------------------------------------------------------------
# 6. Compile and run the download demo
# ---------------------------------------------------------------------------
DEMO_DIR="$REPO_DIR/demos/WebViewDownloadDemo"
DEMO_CLASSES="$BUILD_DIR/classes-download-demo"
mkdir -p "$DEMO_CLASSES"
echo "Compiling demo ..."
"$JAVAC" -d "$DEMO_CLASSES" -classpath "$WV_JAR" \
    "$DEMO_DIR/src/ca/weblite/webview/demos/WebViewDownloadDemo.java"

echo "Launching WebViewDownloadDemo (mode=$MODE) ..."
...
exec "$JAVA" \
    ...
    ca.weblite.webview.demos.WebViewDownloadDemo
