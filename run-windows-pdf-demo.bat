@echo off
REM ============================================================================
REM run-windows-pdf-demo.bat
REM
REM One-shot build & launch of WebViewPdfDemo on Windows (Canvas 29).  Builds
REM the WebView2-backed native DLL, packages WebView.jar, compiles the demo, and
REM runs it.  No Ant required.
REM
REM Prints a two-page report (dark full-bleed cover + plain page) to
REM %USERPROFILE%\webview-pdf-demo.pdf (Letter) and webview-pdf-demo-a4.pdf
REM (A4, 0.5in margins) from the heavyweight component, the supported Windows
REM mode.  Set PDFDEMO_AUTO=1 to print, check and exit (0 = passed).  See
REM demos\WebViewPdfDemo\README.md for the AC-mapped manual test checklist.
REM
REM Requires:
...
REM ---------------------------------------------------------------------------
REM 7. Compile and launch the PDF demo
REM ---------------------------------------------------------------------------
set "DEMO_DIR=%REPO_DIR%\demos\WebViewPdfDemo"
set "DEMO_CLASSES=%BUILD_DIR%\classes-pdf-demo"
if not exist "%DEMO_CLASSES%" mkdir "%DEMO_CLASSES%"
echo Compiling demo ...
"%JAVA_HOME%\bin\javac.exe" -d "%DEMO_CLASSES%" -classpath "%WV_JAR%" "%DEMO_DIR%\src\ca\weblite\webview\demos\WebViewPdfDemo.java"
if errorlevel 1 (
    echo ERROR: javac failed on demo.
    exit /b 1
)

echo Launching WebViewPdfDemo ...
set "PDFDEMO_AUTO_FLAG=false"
if defined PDFDEMO_AUTO set "PDFDEMO_AUTO_FLAG=true"
"%JAVA_HOME%\bin\java.exe" "-Dpdfdemo.auto=%PDFDEMO_AUTO_FLAG%" -cp "%DEMO_CLASSES%;%WV_JAR%" ca.weblite.webview.demos.WebViewPdfDemo
