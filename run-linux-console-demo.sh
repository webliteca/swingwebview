...
# and run the console + DevTools Swing demo.  No Ant required.
#
# Usage:    ./run-linux-console-demo.sh
# Override: JAVA_HOME=/path/to/jdk ./run-linux-console-demo.sh
#
# Required packages:
#   - g++, pkg-config
#   - libgtk-3-dev
...
# ---------------------------------------------------------------------------
# 6. Compile and run the console demo
# ---------------------------------------------------------------------------
DEMO_DIR="$REPO_DIR/demos/WebViewConsoleDemo"
DEMO_CLASSES="$BUILD_DIR/classes-demo"
mkdir -p "$DEMO_CLASSES"
echo "Compiling demo ..."
"$JAVAC" -d "$DEMO_CLASSES" -classpath "$WV_JAR" \
    "$DEMO_DIR/src/ca/weblite/webview/demos/WebViewConsoleDemo.java"
...
exec "$JAVA" \
    ...
    ca.weblite.webview.demos.WebViewConsoleDemo
