@echo off
REM ============================================================================
REM run-windows-console-demo.bat
REM
REM One-shot build & launch of the console + DevTools Swing demo on Windows.  Builds
...
if "%JAVA_HOME%"=="" (
    ...
    echo     run-windows-console-demo.bat
    echo.
    echo If you use mise, "mise install" will read .tool-versions and you can
    echo "mise exec -- run-windows-console-demo.bat".
    echo.
    exit /b 1
)
...
REM ---------------------------------------------------------------------------
set "DEMO_DIR=%REPO_DIR%\demos\WebViewConsoleDemo"
set "DEMO_CLASSES=%BUILD_DIR%\classes-demo"
if not exist "%DEMO_CLASSES%" mkdir "%DEMO_CLASSES%"
echo Compiling demo ...
"%JAVA_HOME%\bin\javac.exe" -d "%DEMO_CLASSES%" -classpath "%WV_JAR%" "%DEMO_DIR%\src\ca\weblite\webview\demos\WebViewConsoleDemo.java"
if errorlevel 1 (
...
echo Launching demo ...
"%JAVA_HOME%\bin\java.exe" -cp "%DEMO_CLASSES%;%WV_JAR%" ca.weblite.webview.demos.WebViewConsoleDemo
