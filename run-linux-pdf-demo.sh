...
# and run the print-to-PDF demo (WebViewPdfDemo, Canvas 29).
#
# Prints a two-page report (dark full-bleed cover + plain page) to
# ~/webview-pdf-demo.pdf (Letter) and ~/webview-pdf-demo-a4.pdf (A4, 0.5in
# margins).  See demos/WebViewPdfDemo/README.md for the AC-mapped checklist.
#
# Usage:    ./run-linux-pdf-demo.sh               # lightweight, the supported
#                                                 # Linux mode (Canvas 29 D13)
#           PDFDEMO_AUTO=1 ./run-linux-pdf-demo.sh  # print, check, exit (0 = passed)
#
# Override JDK:  JAVA_HOME=/path/to/jdk ./run-linux-pdf-demo.sh
#
# Required packages:
#   - g++, pkg-config
#   - libgtk-3-dev
...
# ---------------------------------------------------------------------------
# 6. Compile and run the PDF demo
# ---------------------------------------------------------------------------
DEMO_DIR="$REPO_DIR/demos/WebViewPdfDemo"
DEMO_CLASSES="$BUILD_DIR/classes-pdf-demo"
mkdir -p "$DEMO_CLASSES"
echo "Compiling demo ..."
"$JAVAC" -d "$DEMO_CLASSES" -classpath "$WV_JAR" \
    "$DEMO_DIR/src/ca/weblite/webview/demos/WebViewPdfDemo.java"

echo "Launching WebViewPdfDemo (mode=$MODE) ..."
...
exec "$JAVA" \
    ...
    "-Dpdfdemo.auto=${PDFDEMO_AUTO:+true}" \
    -cp "$DEMO_CLASSES:$WV_JAR" \
    ca.weblite.webview.demos.WebViewPdfDemo
