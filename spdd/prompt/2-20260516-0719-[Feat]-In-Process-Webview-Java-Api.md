...
- Provide a fluent Java API (`ca.weblite.webview.WebView`) to open a
  ...
  loop (`WebView.java:43`).
- Callers must be able to set: URL, window title, size (width x
  height), resizable flag, and one or more `addOnBeforeLoad(js)`
  snippets that run on every navigation (`WebView.java:62`,
  `WebView.java:107`, `WebView.java:128`, `WebView.java:143`,
  `WebView.java:85`).
- Callers must be able to register named JavaScript callbacks that
  ...
  string argument (`WebView.java:178`).
- Callers must be able to run arbitrary JavaScript via `eval(js)`
  after the window is shown (`WebView.java:203`) and dispatch
...
- `show()` is the terminal call: it creates the native peer, applies
  ...
  (`WebView.java:229`).
- Usage example: `README.md:104` (the "Using Java API" section).
- Definition of Done: no automated tests cover this class directly;
  `WebViewMinimalDemo` (`demos/WebViewMinimalDemo/...:20`) is the
  smoke test — launching the minimal demo must open a window.
- Known limitation documented in the class Javadoc
  ...
  nicely with other GUI toolkits. Callers needing Swing/JavaFX
  integration must use [[subprocess-webview-client]] or the
  [[swing-webview-component-mode-selection]] embedding API instead.

## E · Entities
- **WebView** (`WebView.java:43`) — the high-level wrapper around a
  zserge-style WebView native peer. Holds a `long peer` (native
  pointer, 0 until `show()` is called — `WebView.java:47`), buffered
  configuration (URL, title, size, resizable, fullscreen — defaults
  at `WebView.java:50`–`WebView.java:54`), pending init scripts
  (`onBeforeLoad: List<String>` — `WebView.java:55`), and the binding
  map `bindings: Map<String, JavascriptCallback>`
  (`WebView.java:56`).
- **WebView.JavascriptCallback** (`WebView.java:71`) — a functional
...
- **heap** (`WebView.java:169`) — `ArrayList` used to anchor native
...
- **Two-phase configuration.** Setter methods buffer their values
  ...
  (`WebView.java:107` for URL, `WebView.java:128` for title,
  `WebView.java:178` for bindings, `WebView.java:85` for init JS).
...
- **Single event loop, single thread.** `show()` calls
  ...
  window closes (`WebView.java:247`). Concurrent Java work must be
  marshalled via `dispatch(Runnable)` (`WebView.java:215`).
...
- `src/ca/weblite/webview/WebViewNativeCallback.java` — native
  ...
  JavaScript callback.
- `demos/WebViewMinimalDemo/src/.../WebViewMinimalDemo.java` —
  minimal usage example (`WebViewMinimalDemo.java:20`).
...
2. Methods:
   ...
   - `resizable(boolean b): WebView`
     - Logic: store on `resizable` field; not currently forwarded to
       ...
       always `0` — `WebView.java:231`). `[DRIFT]` from the
...
2. Methods:
   - `addJavascriptCallback(String name, JavascriptCallback callback): WebView`
     - Logic: put `name → callback` into `bindings`
       (`WebView.java:180`). If `peer != 0`, construct a
...
2. Methods:
   - `dispatch(Runnable r): WebView`
     - Logic: add `r` to `heap` (anti-GC), call
       ...
       heap.remove(r); }, 0)` (`WebView.java:215`).
3. Constraints / Invariants:
...
2. Methods:
   - `show(): void`
     - Logic: assign
       ...
       (`WebView.java:230`); set bounds via
       `webview_set_bounds(peer, 0, 0, w, h, 0)`
       (`WebView.java:231`); replay each init JS via
       `webview_init(peer, js)` (`WebView.java:232`); set the title
       via `webview_set_title(peer, title)` (`WebView.java:235`);
       for every binding, create a `WebViewNativeCallback`
       lambda, anchor it in `heap`, and call
       `webview_bind(peer, key, fn, peer)`
       (`WebView.java:236`–`WebView.java:245`); navigate via
       `webview_navigate(peer, url)` (`WebView.java:246`); finally
       call `webview_run(peer)` (`WebView.java:247`), which blocks
       until the window closes.
3. Constraints / Invariants:
   - On macOS, the calling JVM **must** have been started with
     `-XstartOnFirstThread` or this method will fail / misbehave;
     `WebViewCLI.restartJVM` and the demo scripts ensure this. See
     [[webview-cli-application]] section 2.
...
- Java 8 source/target (`pom.xml:41`). Anonymous-inner-class style
  ...
  (`WebView.java:183`) to stay debuggable on older JDKs.
...
- The `heap` field anchors live callbacks so GC cannot collect them
  ...
  (`WebView.java:169`, `WebView.java:192`, `WebView.java:216`).
...
- `addJavascriptCallback` checks `peer == 0` to decide between
  buffering and immediate bind (`WebView.java:179`) — but the
  buffered branch never adds to `heap` because the `show()` path
  re-creates a fresh callback later (`WebView.java:236`).
